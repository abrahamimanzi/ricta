...
//		$user_ID = Session::get(Config::get('session/session_name'));
		...
		if(!$this->_db->insert('events_participant_item', $fields)){
...
// USER DATA UPDATE
	public function update($fields = array(),$id = null){
		if(!$this->_db->update('events_participant_item',$id,$fields)){
...
// FIND USER
	public function find($user = null,$limit = null){
		if($user){
			...
			if(is_numeric($user)){
				$field = 'ID';
				$data = $this->_db->get('events_participant_item', array($field, '=', $user),$limit);
				if($data->count()){
	...
	public function select($sql = null){
		$data = $this->_db->query("SELECT* FROM `events_participant_item` {$sql}");
		if($data->count()){
