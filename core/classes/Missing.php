...
//USER CREATE AN ACCOUNT
	public function insert($fields = array()){
		...
		if(!$this->_db->insert('missing', $fields)){
			throw new Exception("There was a problem creating your entry.");
		}
	}
...
// USER DATA UPDATE
	public function update($fields = array(),$id = null){
		if(!$this->_db->update('missing',$id,$fields)){
...
// FIND USER
	public function find($user = null,$limit = null){
		if($user){
			...
			if(is_numeric($user)){
				$field = 'ID';
				$data = $this->_db->get('missing', array($field, '=', $user),$limit);
				if($data->count()){
	...
	public function select($sql = null){
		$data = $this->_db->query("SELECT* FROM `missing` {$sql}");
		if($data->count()){
