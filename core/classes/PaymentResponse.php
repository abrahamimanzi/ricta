...
//		$user_ID = Session::get(Config::get('session/session_name'));
		...
		if(!$this->_db->insert('payment_receive', $fields)){
...
// USER DATA UPDATE
	public function update($fields = array(),$id = null){
		if(!$this->_db->update('payment_receive',$id,$fields)){
...
// FIND USER
	public function find($user = null,$limit = null){
		if($user){
			...
			if(is_numeric($user)){
				$field = 'ID';
				$data = $this->_db->get('payment_receive', array($field, '=', $user),$limit);
				if($data->count()){
	...
	public function select($sql = null){
		$data = $this->_db->query("SELECT* FROM `payment_receive` {$sql}");
		if($data->count()){
