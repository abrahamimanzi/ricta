...
//USER CREATE AN ACCOUNT
	public function insert($fields = array()){
		...
		if(!$this->_db->insert('facegorilla_form', $fields)){
...
// USER DATA UPDATE
	public function update($fields = array(),$id = null){
		if(!$this->_db->update('facegorilla_form',$id,$fields)){
...
// FIND USER
	public function find($user = null,$limit = null){
		if($user){
			...
			if(is_numeric($user)){
				$field = 'ID';
				$data = $this->_db->get('facegorilla_form', array($field, '=', $user),$limit);
				if($data->count()){
	...
	public function select($sql = null){
		$data = $this->_db->query("SELECT* FROM `facegorilla_form` {$sql}");
		if($data->count()){
