              ...
              <div>
                ...
                <div class="box">
                    ...
                    <?php
                        ...
                        $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `payment_method`='Bank-Transfer' AND `payment_state`='Pending' AND `state`!=? AND `event_ID`=? $search_sql ",$sql_val_array);
                        $participantApprovalNumber = $participantTable ->count();
                    ?>
                      <h3 class="box-title">Pending bank transfer <span style='color:#01aef0'><?=$participantApprovalNumber?></span></h3>
                        ...
                        <table class="table no-margin">
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $participantTable = new Participant();
								$participantTable->selectQuery("SELECT* FROM `events_participant` WHERE `payment_method`='Bank-Transfer' AND `payment_state`='Pending' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('keyword','get','0') || strlen(Input::get('keyword','get'))<3){
                                ...
                                $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `payment_method`='Bank-Transfer' AND `payment_state`='Pending' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
                                                ...
                                                <form method="post">
                                                    ...
                                                    <ul class="popover-menu-list">
                                                        ...
                                                        if($session_user_data->groups == 'Admin' || $session_user_data->groups == 'RG-Admin' || $session_user_data->groups == 'RG-SUPER-Admin'){?>
                                                            ...
                                                            <?php if($participant_data->state!='Confirmed' || ($participant_data->state!='Confirmed' AND $participant_data->category=='Government' AND $participant_data->payment_state=='Confirmed')){?>
                                                            <li> 
                                                        ...
                                                        <?php
                                                        if(($session_user_data->groups == 'Admin' || $session_user_data->groups == 'Admin-User' || $session_user_data->groups == 'Super-Admin-User' || $session_user_data->groups == 'OGS-User') && ($participant_data->pass_type == 'Silver' ||$participant_data->pass_type == 'Gold' || $participant_data->pass_type == 'Platinum' || $participant_data->category == 'Exhibitor')){?>
