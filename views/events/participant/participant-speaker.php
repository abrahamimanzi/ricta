              ...
              <div>
                ...
                <div class="box">
                    ...
                    <?php
                        ...
                        $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `category`='Speaker-Applicant' AND `state`!=? AND `event_ID`=? $search_sql ",$sql_val_array);
                        $participantApprovalNumber = $participantTable ->count();
                    ?>
                      <h3 class="box-title">Speakers list <span style='color:#01aef0'><?=$participantApprovalNumber?></span></h3>
                        ...
                        <table class="table no-margin">
                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $participantTable = new Participant();
								$participantTable->selectQuery("SELECT* FROM `events_participant` WHERE `category`='Speaker-Applicant' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('keyword','get','0') || strlen(Input::get('keyword','get'))<3){
                              ...
                              $participantTable = new Participant();
                              $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `category`='Speaker-Applicant' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
								...
								if($participantTable->count()){
									...
									foreach($participantTable->data() as $participant_data){
                                        ...
                                        if(($participant_data->payment_state == 'Confirmed' || 
                                            ...
                                            $participant_data->category == 'Speaker-Applicant'|| 
                                              ...
                                              <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                  ...
                                                  <div class="modal-footer" style="text-align: left">
                                                    ...
                                                    <?php if($participant_data->category == "Speaker-Applicant" || $participant_data->category == "Media"){
                    ...
                    <div class="box-footer clearfix">
                        ...
                        <?php  
                            ...
                            <a href="<?=DNADMIN?>/app/EventApp/<?=$event_ID?>/participant-list-exportspeaker" class="btn btn-sm btn-primary btn-flat pull-right">Export</a>
                        <?php } ?>
                    </div>
                    <!-- /.box-footer -->
                  </div>
