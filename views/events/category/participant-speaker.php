                          ...
                          <tbody>
                            ...
                            $seconds = \Config::get('time/seconds');
                              $participantTable = new Participant();
								$participantTable->selectQuery("SELECT* FROM `events_participant` WHERE `category`='Speaker' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']}",$sql_val_array);
                            ...
                            if(Input::checkInput('keyword','get','0') || strlen(Input::get('keyword','get'))<3){
                              ...
                              $participantTable = new Participant();
                              $participantTable->selectQuery("SELECT * FROM `events_participant` WHERE `category`='Speaker' AND `state`!=? AND `event_ID`=? $search_sql ORDER BY {$_SESSION['participant_sort']} LIMIT {$offsetNumber},{$rowsLimit}",$sql_val_array);
								...
								if($participantTable->count()){
									...
									foreach($participantTable->data() as $participant_data){
                                        ...
                                        if($participant_data->category == 'Visitor'){
                                            $deadline = 10;
                                        }elseif($participant_data->category == 'Media'){
                                            $deadline = 5;
                                        }else{
                                            $deadline = 3;
                                        }
                                                ...
                                                <form method="post">
                                                    ...
                                                    <ul class="popover-menu-list">
                                                        ...
                                                        if($session_user_data->groups == 'Admin' || $session_user_data->groups == 'RG-Admin' || $session_user_data->groups == 'RG-SUPER-Admin'){?>
                                                            ...
                                                            <?php if($participant_data->state!='Confirmed'){?>
                                                            <li> 
                                                        ...
                                                        <?php
                                                        if(($session_user_data->groups == 'Admin' || $session_user_data->groups == 'Admin-User' || $session_user_data->groups == 'Super-Admin-User') && ($participant_data->category == 'Delegate' || $participant_data->category == 'Exhibitor')){?>
